<template>
  <div class="editor">
    ...
    <!-- 富文本编辑器 -->
    <div v-show="article.type === 0">
      <transition name="el-fade-in-linear">
        <quillEditor v-model="article.content" ref="vueQuillEditor" :options="editorOption" @blur="onEditorBlur($event)"
          @focus="onEditorFocus($event)" @change="onEditorChange($event)">
        </quillEditor>
      </transition>
    </div>
    <!-- Markdown编辑器 -->
    <div v-show="article.type === 1">
      <transition name="el-fade-in-linear">
    ...
    <el-dialog title="发布文章" :visible.sync="dialogVisible" width="30%">
      <el-form :label-position="'left'" :model="article" :rules="rules" ref="article" label-width="90px">
        <el-form-item label="封面图片" prop="coverImage">
          <el-upload :action="upLoadUrl" :http-request="uploadCoverImage" :file-list="fileList"
            name="image"
            list-type="picture">
  ...
  export default {
    ...
    data() {
      return {
        ...
        dialogVisible: false,
        upLoadUrl: "http://localhost:8080/file/uploadImage",
        fileList: [],
        editorOption: {},
      }
    },
