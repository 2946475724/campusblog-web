...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      selectChange(value) {
      console.log(value);
      for (let i = 0; i < this.categories.length; i++) { if (this.categories[i].name===value) {
        this.article.categoryId=this.categories[i].id; console.log(this.article.categoryId); } } }, //标签输入
        handleClose(tag) { this.dynamicTags.splice(this.dynamicTags.indexOf(tag), 1); }, showInput() {
        this.inputVisible=true; this.$nextTick(_=> {
