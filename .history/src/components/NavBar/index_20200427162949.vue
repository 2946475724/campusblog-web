  ...
  <div class="header-container">
    <header class="header">
      ...
      <ul class="header-list">
        <li v-for="navItem in nav" @click="selectNav(navItem.name)">
          <router-link to="navItem.url">
            <a href="javascript:void(0);" :class="{active:activeName === navItem.name}">{{navItem.name}}</a>
          </router-link>
        </li>
      </ul>
        ...
        <div v-if="loginVisible" class="login-box">
          ...
          <el-form label-position="right" :model="user" :rules="rules" ref="user" class="login-box-form">
            ...
            <el-form-item prop="password">
              <el-input type="password" v-model="user.password" auto-complete="new-password" placeholder="请输入密码">
              </el-input>
            </el-form-item>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        nav: [{name: '首页', url: '/'}, {name: '博客', url: '/article'}],
        activeName: '',
        loginVisible: false,
        dialogVisible: false,
        isLogin: false,
