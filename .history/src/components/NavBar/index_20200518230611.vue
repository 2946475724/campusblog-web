<template>
  <div class="header-container">
    <header class="header">
      ...
      <div class="logo">
        <router-link to="/">
          <a href="javascript:void(0);"><img src="/static/images/logo1.png" /></a>
        </router-link>
      </div>
      <!-- 导航栏 -->
      <ul class="header-list">
        <li v-for="(navItem, index) in nav" :key="index">
          <router-link :to="navItem.url" active-class="active" exact>
            {{navItem.name}}
          </router-link>
        </li>
      </ul>
      <div class="search-box">
        <input type="text" class="" placeholder="搜一搜" name="" v-model="keyword" v-on:keyup.enter="search" />
      ...
      <div class="user-box">
        <div v-if="!isLogin">
          <span @click="gotoLogin" class="login">登录</span>
          <!-- <span>|</span> -->
          <span @click="gotoSignUp" class="register">注册</span>
        </div>
      ...
      <el-dialog width="23%" :visible.sync="dialogVisible" :append-to-body='true' :lock-scroll="false">
        ...
        <div v-if="loginVisible" class="login-box">
          <span slot="title" class="login-box-title">
            <h2>登录</h2>
            <span class="login-tip">没有账号? 请点击 <span class="login-tip-click" @click="gotoSignUp">注册</span></span>
          </span>
          <el-form label-position="right" :model="user" :rules="rules" ref="user" class="login-box-form">
            ...
            <el-form-item prop="password">
              <el-input type="password" v-model="user.password" auto-complete="new-password" show-password
                placeholder="请输入密码">
              </el-input>
            </el-form-item>
