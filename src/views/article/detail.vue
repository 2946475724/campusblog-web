        ...
        <article class="info">
          <div class="detail">
            ...
            <div class="article-metas">
              <span class="meta-item" style="cursor: pointer;" @click="gotoUserInfo()">
            ...
            <div class="tool-box">
              <span class="tool-box-item" @click="like">
                <svg-icon v-if="this.token != undefined && likeStatus === 1" icon-class="like-active" class="icon-size">
                </svg-icon>
                <svg-icon v-else icon-class="like" class="icon-size"></svg-icon>
                点赞{{articleInfo.likes}}
              </span>
              <span class="tool-box-item" @click="collection">
                <svg-icon v-if="this.token != undefined && collectionStatus === 1" icon-class="collection-active"
  ...
  export default {
    ...
    methods: {
      ...
      getLikeStatus() {
        ...
        getLikeStatus(data).then(resp => {
          this.likeStatus = resp.data
        })
      },
      // 点赞
      like() {
      ...
      getCollectionStatus() {
        ...
        getCollectionStatus(data).then(resp => {
          this.collectionStatus = resp.data
        })
      },
      // 收藏
      collection() {
      ...
      //跳转到用户主页
      gotoUserInfo() {
        let id = this.articleInfo.userId
        this.$router.push({
          path: `/user/${id}`,
        })
      },
